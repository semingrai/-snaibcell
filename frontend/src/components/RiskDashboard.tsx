...
export default function RiskDashboard({ patient, prediction, isDark, onNewPatient }: Props) {
  ...
  const urgBg = {
    ...
    YELLOW: isDark ? 'bg-amber-500/8 border-amber-500/20'  : 'bg-amber-50 border-amber-200',
    GREEN:  isDark ? 'bg-green-500/8 border-green-500/20'  : 'bg-green-50 border-green-200',
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex flex-col gap-7 flex-1 px-12 py-9 overflow-y-auto">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className={`flex items-center gap-2 px-3 py-1.5 border ${urgBg}`}>
              <span className={`w-2 h-2 rounded-full ${urgText.replace('text', 'bg')} animate-pulse`} />
              <span className={`text-[11px] font-semibold ${urgText}`}>{urgMsg}</span>
            </div>
          </div>
        ...
        {/* Right panel */}
        <aside className={`w-[380px] shrink-0 flex flex-col p-8 border-l overflow-y-auto ${rp} ${rpBdr}`}>
          <RiskFactors prediction={prediction} isDark={isDark} />
        </aside>
      </div>
    </div>
  )
